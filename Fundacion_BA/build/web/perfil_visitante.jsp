<%-- 
    Document   : perfil_visitante
    Created on : 25-ene-2022, 15:12:58
    Author     : Andrés Marín
--%>
...
<html lang="en">
                        ...
                        <center>
                            <div class="input-group">
                                <span class="input-group-text" id="basic-addon1" style="background:#23BFD8;align-content: center;border-color: #23BFD8">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="55" height="55" fill="currentColor" class="bi bi-emoji-smile" viewBox="0 0 16 16">
                                    <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                    <path d="M4.285 9.567a.5.5 0 0 1 .683.183A3.498 3.498 0 0 0 8 11.5a3.498 3.498 0 0 0 3.032-1.75.5.5 0 1 1 .866.5A4.498 4.498 0 0 1 8 12.5a4.498 4.498 0 0 1-3.898-2.25.5.5 0 0 1 .183-.683zM7 6.5C7 7.328 6.552 8 6 8s-1-.672-1-1.5S5.448 5 6 5s1 .672 1 1.5zm4 0c0 .828-.448 1.5-1 1.5s-1-.672-1-1.5S9.448 5 10 5s1 .672 1 1.5z"/>
                                    </svg>&nbsp;&nbsp;&nbsp;
                                    <h2 style="font-weight: 700">¡Hola <%=session.getAttribute("datosUser")%>! </h2>
                                </span>
                            </div>
                        </center>
                    </div>
    ...
    <%
        ArrayList<Mascota> datos3 = null;
        datos3 = (ArrayList<Mascota>) request.getAttribute("datos");%>
    <div class="container p-5">
        <div class="row">
            <div class="col-md-6 p-0 m-auto">
                <img src="images/FBA2.png" class="img-border" alt="Avatar"/></img>
            </div>
            <div class="col-md-6">
                <h1 class="font-weight-bold text-center text-uppercase p-sm-3"><%=session.getAttribute("datosUser")%> <%=session.getAttribute("primerApellido")%> <%=session.getAttribute("segApellido")%></h1>

                <div class="m-2">
                    <h3><span class="font-weight-bold">Fecha de nacimiento: </span><%=session.getAttribute("fecha")%></h3>
                    <h3><span class="font-weight-bold">Correo electrónico: </span><%=session.getAttribute("email")%></h3>
                    <!--<h3><span class="font-weight-bold">Estado civil: </span><%=session.getAttribute("barrio")%></h3>
                    <h3><span class="font-weight-bold">Barrio de residencia: </span><%=session.getAttribute("estado_civil")%></h3>-->
                    <h3><span class="font-weight-bold">Dirección: </span><%=session.getAttribute("direccion")%></h3>
                    <h3><span class="font-weight-bold">Celular: </span><%=session.getAttribute("celular")%></h3>
                    <h3><span class="font-weight-bold">Profesión u oficio: </span><%=session.getAttribute("puesto")%></h3>
                </div>

                <button type="submit" class="btn btn-info m-2"><a class="text-white" href="edit_perfil.jsp" style="font-size:18px;font-weight: 700" data-toggle="modal" data-target="#exampleModal"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>&nbsp; Editar información</a></button>

                <form method="POST" action="ControladorVisitante">
                    <button type="submit" value="cambiarcontra" name="visitante" class="btn btn-dark m-2" style="font-size:18px;font-weight: 700">
                        <a class="d-flex align-items-center text-white"><i class="fa fa-pencil" aria-hidden="true"></i>&nbsp; Cambiar contraseña
                        </a>
                    </button>
                </form>

                <!--<form method="POST" action="ControladorIniciosesion">
                    <input type="hidden" name="idDesencrip" value="<%=session.getAttribute("idDesencriptado")%>">
                    <button type="submit" class="btn btn-danger m-2" name="accion" value="eliminarMyCountVisitante" style="font-size:18px;font-weight: 700"><i class="fa fa-trash-o" aria-hidden="true"></i>&nbsp; Eliminar cuenta</button> 
                </form>-->
                    ...
                    <div class="modal-dialog modal-lg" role="document" >
                        <div class="modal-content" >
                            <div class="modal-header">
                                <div class="input-group">
                                    <span class="input-group-text" id="basic-addon1">
                                        ...
                                        <h5 class="modal-title text-info font-weight-bold text-center" id="exampleModalLabel">&nbsp; &nbsp;Editar mi información</h5>
                                    </span>
                                </div>
                            ...
                            <div class="modal-body">
                                <form onsubmit="validarPerfil()" action="ControladorIniciosesion" method="POST" name="form_perfil" onsubmit="formRegistroUsuario(event)" >
                                    <div class="form-group">
                                        <label for="message-text" class="col-form-label font-weight-bold">Nombre<span class="text-danger"> *</span> </label>
                                        <input type="tel" class="form-control" id="recipient-name" name="Nombre" value="<%=session.getAttribute("datosUser")%>"/>
                                    </div>

                                    <div class="form-group">
                                        <label for="message-text" class="col-form-label font-weight-bold">Primer apellido<span class="text-danger"> *</span> </label>
                                    ...
                                    <div class="form-group">
                                        <label for="recipient-name" class="col-form-label font-weight-bold">Barrio<span class="text-danger"> *</span> </label>
                                        <div class="col-md-12">
                                            <select id="sel1" name="select_barrio" required>
                                    <%for (int i = 0; i < lista.size(); i++) {%>
                                    <%if (String.valueOf(session.getAttribute("barrio")).equals(String.valueOf(lista.get(i).getId()))) {%>
                                    <option value="<%=lista.get(i).getId()%>" selected="true"><%=lista.get(i).getNombre_barrio()%></option> 
                                    <%} else {%>
                                    ...
                                    <div class="form-group">
                                        <label for="recipient-name" class="col-form-label font-weight-bold">Estado civil<span class="text-danger"> *</span> </label>
                                        <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        <label for="message-text" class="col-form-label font-weight-bold">Dirección de residencia<span class="text-danger"> *</span></label>
                                        <input type="text" class="form-control" id="recipient-name" name="direccion" value="<%=session.getAttribute("direccion")%>">
                                    </div>

                                    <div class="form-group">
                                        <label for="message-text" class="col-form-label font-weight-bold">Celular<span class="text-danger"> *</span> </label>
                                        <input type="tel" class="form-control" id="recipient-name" name="celular" value="<%=session.getAttribute("celular")%>">
                                    </div>

                                    <div class="form-group">
                                        <label for="message-text" class="col-form-label font-weight-bold">Correo eletronico<span class="text-danger"> *</span> </label>
                                        <input type="email" class="form-control" id="recipient-name" name="email" value="<%=session.getAttribute("email")%>">
                                    </div>

                                    <div class="form-group">
                                        <label for="message-text" class="col-form-label font-weight-bold" >Profesión u oficio<span class="text-danger"> *</span> </label>
                                    ...
                                    <div class="row" style="margin-left: 15px">
                                        ...
                                        <input type="hidden" name="idDesencrip" value="<%=session.getAttribute("idDesencriptado")%>"/>
                                        <button type="submit" class="btn btn-info alinear" name="accion" value="ActualizarPerfil">
