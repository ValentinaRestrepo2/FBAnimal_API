<%-- 
    Document   : informe_visitante
    Created on : 2/07/2022, 02:57:25 PM
    Author     : PC
--%>

<%@page import="Modelo.Informes"%>
<%@page import="Modelo.InformesDAO"%>
<%-- 
    Document   : mascotas_visitante
    Created on : 23-ene-2022, 16:41:13
    Author     : Andrés Marín
--%>
...
<html lang="en">
    <head>
        ...
        <!-- site metas -->
        <title>Informe seguimiento | Fundación Bello Animal</title>
    ...
    <div class="backgro_mh">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="heding">
                        <h2 style="font-weight: 700">Informes de seguimiento</h2>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="fondo6">
        <div class="container"> 
            <div class="row">
                <div class="col-md-12">
                    <form class="contact_bg" action="ControladorVisitante" method="POST" enctype="multipart/form-data" onsubmit="formCrearInformeMascota(event)"  >
                        <div class="col-md-12"> 
                            <br>
                            <h3 class="font-weight-bold" style="font-size: 30px;color: white">Crear informe</h3>
                            <div class="col-md-12 p-0 pt-2">
                                <textarea class="textarea" placeholder="Describe aquí las condiciones actuales de la mascota que adoptaste*" autocomplete="off" type="text" id="descripcion_informe" name="descripcion_informe"></textarea>
                            </div>

                            <div class="col-md-12 p-0 pt-2">
                                <textarea class="textarea" placeholder="¿Tienes alguna sugerencia, pregunta o comentario?" autocomplete="off" type="text" name="mensaje_admin" id="mensaje_admin"></textarea>
                            </div>
                            <div class="form-group col-md-12 p-0 pt-2">
                                <select class="form-control" style="margin-left: 50px;width:85%;background-color: white" id="mascota_informe" name="mascota_informe" autocomplete="off">
                                    <option value="0" selected disabled="true">--Selecciona la mascota a informar --</option>
                                    <%
                                        List<Informes> datos3;
                                        datos3 = null;
                                        datos3 = informeDAO.misMascotasAdoptasLista(id);

                                        List<Informes> datos;
                                        datos = null;
                                        datos = informeDAO.misMensajesEnviados(id);
                                        for (Informes info : datos3) {%>
                                    %>
                                    <option value="<%=info.getIdMascota()%>"><%=info.getNombreMascota()%></option>
                                    <% }%>%>
                                </select>
                            </div><br>
                            <div class="col-md-12 p-0 pt-2">
                                <label class="foto_informe" for="foto_informe" style="margin-left: 20px;width:85%;font-size: 17px">   
                                    Sube una foto de la mascota &nbsp;&nbsp; &nbsp; &nbsp;  
                                    <input type="file" name="foto_informe" id="foto_informe" accept="image/png,image/jpeg" >
                                </label>
                            </div>
                            ...
                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 mt-5">
                                <button type="submit" name="visitante" value="enviar_informe" class="sendlogin" style="font-size:18px;font-weight: 700">Enviar informe</button>
                            </div>
                        </div>
                        <br>
                        <br>
                        ...
                        <table class="tabla2 table table-responsive table-hover" id="datos">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center color-azul font-weight-bold">MENSAJE AL ADMIN</th>
                                    <th class="text-center color-azul font-weight-bold">MASCOTA</th>
                                    <th class="text-center color-azul font-weight-bold">FECHA</th>
                                    <th class="text-center color-azul font-weight-bold">RESPUESTA DEL ADMIN</th>
                                    <!--<th class="text-center color-azul font-weight-bold">ACCIÓN</th>-->
                                </tr>
                            </thead>
                            <tbody>
                                <%if (datos != null) {
                                        for (Informes info : datos) {
                                %>
                                <tr>
                                    <td style = "text-align: center;"> <%=info.getId_informe()%></td>
                                    <td style = "text-align: center;"> <%=info.getDescripcion_informe()%></td>   
                                    <td style="text-align: center;"><%=info.getMensaje_informe()%></td> 
                                    <td style="text-align: center;">(<%=info.getIdMascota()%>) <%=info.getNombreMascota()%></td> 
                                    <td style="text-align: center;"><%=info.getFecha_informe()%></td>
                                    <% if (info.getRespuesta_admin_informe() != null) {%>
                                    <td style="text-align: center;" class="font-weight-bold"><%=info.getRespuesta_admin_informe()%></td>
                                    <%} else {%>
                                    <td style="text-align: center;font-size: 17px;font-weight: 700" class="text-danger">No hay respuesta...</td>
                                    <%
                                        }
                                    %>
                                    <!--<td style="text-align: center;" class="d-flex align-content-center justify-content-center align-items-center">
                                        <form id="form_detele_mascota" method="POST" action="ControladorMascota">
                                            <input type="hidden" name="id" value="<%=info.getId_informe()%>">
