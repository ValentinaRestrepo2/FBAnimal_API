<%-- 
    Document   : listarContactos_admin
    Created on : 14/02/2022, 02:46:26 AM
    Author     : Andrés Marín
--%>
...
<html lang="en">
    ...
    <head>
        ...
        <!-- site metas -->
        <title>Fundación Bello Animal | Mensajes de los usuarios desde la sección contactos</title>
        ...
        <div class="backgro_mh">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class="heding">
                            <h2 style="font-weight: 700">Mensajes recibidos</h2>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="container">
                <div class="row">
                    <%
                        ...
                        List<Contacto> datos;
                        datos = null;
                        datos = cdao.listarContacto();

                        if (datos != null) {
                            for (Contacto contac : datos) {%>
                    <div class="col-md-4">
                        <%
                            if (contac.getSelect_pqrs().equals("2")) {
                                color = "card text-white bg-success mb-3";
                                texto = "Sugerencia";
                                colorTexto = "text-white";
                            } else if (contac.getSelect_pqrs().equals("4")) {
                                color = "card text-white bg-info mb-3";
                                texto = "Pregunta";
                                colorTexto = "text-white";
                            } else if (contac.getSelect_pqrs().equals("3")) {
                                color = "card text-dark bg-warning mb-3";
                                texto = "Reclamo";
                                colorTexto = "text-dark";
                            } else if (contac.getSelect_pqrs().equals("5")) {
                                color = "card text-white bg-danger mb-3";
                                texto = "Quejas";
                                colorTexto = "text-white";
                            }
                        %>
                        <div class="<%=color%> font-weight-bold text-center" style="max-width: 90rem;">
                            <div class="card-header"><%=texto%></div>
                            <div class="card-body text-left">
                                <h5 class="card-title <%=colorTexto%>"><span class="font-weight-bold">Nombre: </span> <%=contac.getNombre()%></h5>
                                <h6 class="card-title <%=colorTexto%>"><span class="font-weight-bold">Celular: </span> <%=contac.getCelular()%></h6>
                                <h6 class="card-title <%=colorTexto%>"><span class="font-weight-bold">Correo: </span> <%=contac.getCorreo()%></h6>
                                <hr>
                                <p class="card-text"><%=contac.getMensaje()%></p>
                            </div>
                            <div>
                                <%
                                    if (contac.getEstado().equals("2")) {%>
                                <div class="bg-dark rounded p-1 text-white">
                                    <p>Ya he contestado</p>
                                </div>
                                <%} else {
                                %>
                                <!--<div class="bg-dark rounded p-1 text-white">
                                    <form action="ControladorContacto" method="POST">
                                        <input type="hidden" name="id" value="<%=contac.getId()%>">
                                        <button class="border-0 backgro-transparente text-white" type="submit" name="accion" value="contestadoContacto" data-toggle="modal" data-target="#exampleModal2" >
                                ...
                                <div class="bg-dark rounded p-1 text-white">
                                    <input type="hidden" name="id" value="<%=contac.getId()%>">
                                    <button class="border-0 backgro-transparente text-white" type="button" data-toggle="modal" data-target="#exampleModal" onclick="cargarDatosContacto('<%=contac.getId()%>', '<%=contac.getCorreo()%>')">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-archive" viewBox="0 0 16 16">
                                        <path d="M0 2a1 1 0 0 1 1-1h14a1 1 0 0 1 1 1v2a1 1 0 0 1-1 1v7.5a2.5 2.5 0 0 1-2.5 2.5h-9A2.5 2.5 0 0 1 1 12.5V5a1 1 0 0 1-1-1V2zm2 3v7.5A1.5 1.5 0 0 0 3.5 14h9a1.5 1.5 0 0 0 1.5-1.5V5H2zm13-3H1v2h14V2zM5 7.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5z"/>
                                        </svg>
                                    </button>
                    ...
                    <h3 class="font-weight-bold text-danger text-center pt-2">No te han contactado en el momento...</h3>
                    <%}%>
                </div>
            </div>
                ...
                <div class="modal-dialog modal-lg" role="document" >
                    <div class="modal-content" >
                        <div class="modal-header">
                            <div class="input-group">
                                <span class="input-group-text" id="basic-addon1">
                                    ...
                                    <h5 class="modal-title text-info font-weight-bold text-center" id="exampleModalLabel"> &nbsp;  Responder mensaje</h5>
                                </span>
                            </div>
                        ...
                        <div class="modal-body">
                            <form  id="formCorreoCon" onsubmit="formResponderPQRS(event)">
                                <div class="form-group">
                                    <label for="message-text" class="col-form-label font-weight-bold">Asunto<span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="recipient-name" name="asunto_contacto_admin" autocomplete="off"/>
                                </div>

                                <div class="form-group">
                                    <label for="message-text" class="col-form-label font-weight-bold">Mensaje a enviar<span class="text-danger">*</span> </label>
                                    <textarea class="form-control" style="height: 150px" name="mail_contacto_admin" autocomplete="off"></textarea>
                                </div>
                                <div class="row" style="margin-left: 15px;align-items: center">
                                    ...
                                    <button type="button" class="btn btn-info alinear"  name="accion" id="enviarCorreoContac" >
